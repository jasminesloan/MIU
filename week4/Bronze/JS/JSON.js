var json = {
	"genre1": {
		"group": ["Group:", "Dirty South"],
		"email": ["Email:", "jasmine47jamieson@gmail.com"],
		"pword": ["Password:", "fullsail"],
		"purchase": ["Purchase:", "Now"],
		"wishlist": ["Wishlist:", "No"],
		"quantity": ["Quantity:", "2"],
		"date": ["Date:", "2012-5-22"],
		"suggestions": ["Suggestions:", "Please add more music"]
	},
	"genre2": {
		"group": ["Group:", "Gospel"],
		"email": ["Email:", "jasminesloan@fullsail.com"],
		"pword": ["Password:", "jasmine"],
		"purchase": ["Purchase:", "Place in Que"],
		"wishlist": ["Wishlist:", "No"],
		"quantity": ["Quantity:", "7"],
		"date": ["Date:", "2012-6-22"],
		"suggestions": ["Suggestions:", "Keep up the good work!"]
	},
	"genre3": {
		"group": ["Group:", "Hip Hop"],
		"email": ["Email:", "jasmine47@gmail.com"],
		"pword": ["Password:", "music"],
		"purchase": ["Purchase:", "Now"],
		"wishlist": ["Wishlist:", "No"],
		"quantity": ["Quantity:", "5"],
		"date": ["Date:", "2012-5-22"],
		"suggestions": ["Suggestions:", "Great music!"]
	},
	"genre4": {
		"group": ["Group:", "Miami Bass"],
		"email": ["Email:", "jtolliver@gmail.com"],
		"pword": ["Password:", "tolliver"],
		"purchase": ["Purchase:", "Place in Que"],
		"wishlist": ["Wishlist:", "Yes"],
		"quantity": ["Quantity:", "9"],
		"date": ["Date:", "2012-06-02"],
		"suggestions": ["Suggestions:", "Hello"]
	},
	"genre5": {
		"group": ["Group:", "Old School"],
		"email": ["Email:", "hello@gmail.com"],
		"pword": ["Password:", "hello"],
		"purchase": ["Purchase:", "Now"],
		"wishlist": ["Wishlist:", "No"],
		"quantity": ["Quantity:", "4"],
		"date": ["Date:", "2012-5-29"],
		"suggestions": ["Suggestions:", "Old School is the best"]
	},
	"genre6": {
		"group": ["Group:", "Oomp Camp Albums"],
		"email": ["Email:", "oompcamp@gmail.com"],
		"pword": ["Password:", "oompcamp"],
		"purchase": ["Purchase:", "Place in Que"],
		"wishlist": ["Wishlist:", "Yes"],
		"quantity": ["Quantity:", "3"],
		"date": ["Date:", "2012-6-19"],
		"suggestions": ["Suggestions:", "VFW"]
	},
	"genre7": {
		"group": ["Group:", "R&B/Slow Jams"],
		"email": ["Email:", "jasminesloan@gmail.com"],
		"pword": ["Password:", "sloan"],
		"purchase": ["Purchase:", "Now"],
		"wishlist": ["Wishlist:", "No"],
		"quantity": ["Quantity:", "8"],
		"date": ["Date:", "2012-5-29"],
		"suggestions": ["Suggestions:", "MMD"]
	},
	"genre8": {
		"group": ["Group:", "Reggae"],
		"email": ["Email:", "jasminejamieson@gmail.com"],
		"pword": ["Password:", "jamieson"],
		"purchase": ["Purchase:", "Place in Que"],
		"wishlist": ["Wishlist:", "Yes"],
		"quantity": ["Quantity:", "7"],
		"date": ["Date:", "2012-06-01"],
		"suggestions": ["Suggestions:", "Great App!"]
	}
}